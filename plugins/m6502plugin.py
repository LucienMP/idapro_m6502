...
"""
    This is a sample plugin for extending gdb support for step-over for the m6502,
...
#--------------------------------------------------------------------------
#
class m6502_stepover_idp_hook_t(idaapi.IDP_Hooks):
    #
    def __init__(self):
    ...
    def ev_calc_step_over(self, target, ip):
        ...
        #
        logging.debug("JSR, checking instruction")
        ta=ida_pro.ea_pointer_frompointer(target)

        insn = idautils.DecodeInstruction(ip)
        if insn and insn.itype in [idaapi.M65_jsr]:
...
#--------------------------------------------------------------------------
class m6502_stepover(idaapi.plugin_t):
    # Processor fix plugin module
    flags = idaapi.PLUGIN_PROC | idaapi.PLUGIN_HIDE
    comment = ""
    wanted_hotkey = ""
    help = "Supports Step-Over for m6502 CPU on JSR, and type information"
    wanted_name = "m6502_stepover"
    ...
    def init(self):
        ...
        logging.debug('Module for m6502 class CPU Step-Overs, and type information')
        ...
        if idaapi.ph_get_id() != idaapi.PLFM_6502: # or idaapi.cvar.inf.filetype != idaapi.f_ELF:
            logging.error("skipped this plugin because CPU being used is not 6502 derivative!")
            return idaapi.PLUGIN_SKIP

        # Connect the IDP hooks
        self.prochook = m6502_stepover_idp_hook_t()
        self.prochook.hook()

        logging.debug( "succeeded, stepover support should now work for JSR!" )
